...
def main(csrf_token, leetcode_session, plans):
    if not leetcode_session and not csrf_token:
        print("Using Non-Premium LeetCode account. Some premium only data will not be returned.")
        print("=====================================================================================================")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Fetch and store LeetCode study plan details.")
    parser.add_argument(
        '--csrf_token', type=str, default=os.getenv("CSRF_TOKEN"), help='CSRF token for authentication'
    )
    parser.add_argument(
        '--leetcode_session', type=str, default=os.getenv("LEETCODE_SESSION"), help='LeetCode session cookie'
    )
    parser.add_argument(
        '--plans', type=str, nargs='+', default=['top-interview-150', 'leetcode-75'], help='Slugs of the study plans to fetch'
    )
