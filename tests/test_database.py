...
class TestDatabase:
    ...
    def test_insert_problem(self, db):
        ...
        with patch('database.database.execute_insert', return_value=1) as mock_execute_insert:
    ...
    def test_insert_study_plan_problem(self, db):
        with patch('database.database.execute_query', return_value=True) as mock_execute_query:
